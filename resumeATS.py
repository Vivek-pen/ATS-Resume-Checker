...
load_dotenv()
# Model configuration is handled in get_llm_output and bert_utils
    ...
    if analyze_button and upload_file is not None:
        with st.spinner("Analyzing your resume..."):
            ...
            if job_description:
                ...
                final_score, score_breakdown = calculate_final_score(pdf_text, job_description)
            ...
            response = get_llm_output(prompt, pdf_text)
            ...
            if match_score is not None:
                st.markdown(f"### 📊 Final Multi-Criteria Score: {final_score:.1f}/100")
                
                # Show breakdown
                c1, c2, c3 = st.columns(3)
                c1.metric("Keyword Match (40%)", f"{score_breakdown.get('Keyword Match', 0):.1f}%")
                c2.metric("Semantic Match (40%)", f"{score_breakdown.get('Semantic Match (BERT)', 0):.1f}%")
                c3.metric("Formatting (20%)", f"{score_breakdown.get('Structure Score', 0):.1f}%")
                
                st.markdown("---")
                st.markdown(f"#### 🗝️ Keyword Analysis (Match: {match_score:.1f}%)")
