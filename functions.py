...
from bert_utils import calculate_bert_similarity
...
def calculate_match_score(resume_text, job_description):
    ...
    # Calculate match percentage (Keyword Score)
    if len(job_words) > 0:
        keyword_match_percentage = (len(matched_keywords) / len(job_words)) * 100
    else:
        keyword_match_percentage = 0
        
    return keyword_match_percentage, matched_keywords, missing_keywords

def calculate_final_score(resume_text, job_description):
    """
    Calculate a comprehensive Multi-Criteria Score.
    Criteria:
    1. Keyword Match (40%)
    2. Semantic (BERT) Match (40%)
    3. Formatting/Structure (20%) - simplified as a check
    """
    if not job_description:
        return 0, {}

    # 1. Keyword Score
    keyword_score, _, _ = calculate_match_score(resume_text, job_description)
    
    # 2. Semantic Score (BERT)
    bert_score = calculate_bert_similarity(resume_text, job_description)
    
    # 3. Structure Score (Simplified)
    # Check for sections
    structure_score = 100
    ...
    # Weights: Keyword 0.4, BERT 0.4, Structure 0.2
    final_score = (keyword_score * 0.4) + (bert_score * 0.4) + (structure_score * 0.2)
    
    breakdown = {
        "Keyword Match": keyword_score,
        "Semantic Match (BERT)": bert_score,
